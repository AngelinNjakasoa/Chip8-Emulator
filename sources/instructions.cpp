...
/* Jump to NNN */
/* Set pc to NNN */
void CPUChip8::ISIChip8::INST_1NNN(uint16_t opcode) {
  cpu.pc = opcode & 0x0FFF;
}

/* Call NNN */
/* Saves pc on top of the stack and jump to NNN */
void CPUChip8::ISIChip8::INST_2NNN(RAMChip8& mmu, uint16_t opcode) {
  ++cpu.sp;
...
void CPUChip8::ISIChip8::INST_3xkk(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_4xkk(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_5xy0(uint16_t opcode) {
...
/* Set Vx to kk */
void CPUChip8::ISIChip8::INST_6xkk(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] = opcode & 0x00FF;
  cpu.IncrementPC(1);
}

/* Set Vx = Vx + kk */
void CPUChip8::ISIChip8::INST_7xkk(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] += opcode & 0x00FF;
  cpu.IncrementPC(1);
}

/* Set Vx = Vy */
void CPUChip8::ISIChip8::INST_8xy0(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] = cpu.v_registers[(opcode & 0x00F0) >> 4];
  cpu.IncrementPC(1);
}

/* Set Vx = Vx or Vy */
void CPUChip8::ISIChip8::INST_8xy1(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] |= cpu.v_registers[(opcode & 0x00F0) >> 4];
  cpu.IncrementPC(1);
}

/* Set Vx = Vx and Vy */
void CPUChip8::ISIChip8::INST_8xy2(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] &= cpu.v_registers[(opcode & 0x00F0) >> 4];
  cpu.IncrementPC(1);
}

/* Set Vx = Vx xor Vy */
void CPUChip8::ISIChip8::INST_8xy3(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] ^= cpu.v_registers[(opcode & 0x00F0) >> 4];
  cpu.IncrementPC(1);
}

/* Set Vx = Vx + Vy, set VF = carry */
void CPUChip8::ISIChip8::INST_8xy4(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] += cpu.v_registers[(opcode & 0x00F0) >> 4];
  cpu.IncrementPC(1);
}

/* Set Vx = Vx - Vy, set VF = carry */
void CPUChip8::ISIChip8::INST_8xy5(uint16_t opcode) {
  cpu.v_registers[(opcode & 0x0F00) >> 8] -= cpu.v_registers[(opcode & 0x00F0) >> 4];
  cpu.IncrementPC(1);
}

/* Set Vx = Vx >> 1, if lsb == 1 then VF = 1 else VF = 0 */
void CPUChip8::ISIChip8::INST_8xy6(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_8xy7(uint16_t opcode) {
  if (cpu.v_registers[(opcode & 0x0F00) >> 8] > cpu.v_registers[(opcode & 0x00F0) >> 4]) {
...
void CPUChip8::ISIChip8::INST_8xyE(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_9xy0(uint16_t opcode) {
  if (cpu.v_registers[(opcode & 0x0F00) >> 8] != cpu.v_registers[(opcode & 0x00F0) >> 4]) {
    cpu.IncrementPC(2);
  }
  cpu.IncrementPC(1);
...
/* Set I = nnn */
void CPUChip8::ISIChip8::INST_ANNN(uint16_t opcode) {
  cpu.I = opcode & 0x0FFF;
  cpu.IncrementPC(1);
}

/* Jump to location nnn + V0 */
void CPUChip8::ISIChip8::INST_BNNN(uint16_t opcode) {
  cpu.pc = (opcode & 0xFFF) + cpu.v_registers[0];
}

/* Set Vx = random_byte and kk */
void CPUChip8::ISIChip8::INST_Cxkk(uint16_t opcode) {
...
/* Set VF = collision */
void CPUChip8::ISIChip8::INST_Dxyn(GPUChip8& gpu, RAMChip8& mmu, uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Ex9E(CUChip8& cui, uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_ExA1(CUChip8& cui, uint16_t opcode) {
  if (cui.keys[cpu.v_registers[(opcode & 0x0F00) >> 8]] == 0) {
    cpu.IncrementPC(2);
    return ;
  }
...
void CPUChip8::ISIChip8::INST_Fx07(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx0A(CUChip8& cui, uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx15(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx18(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx1E(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx29(uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx33(RAMChip8& mmu, uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx55(RAMChip8& mmu, uint16_t opcode) {
...
void CPUChip8::ISIChip8::INST_Fx65(RAMChip8& mmu, uint16_t opcode) {
  uint8_t nb_register = ((opcode & 0x0F00) >> 8);
  for (uint i = 0; i <= nb_register; ++i) {
